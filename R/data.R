...
data.gen = function(seed, sample_size, p, n_g_non_zero, n_gxe_non_zero, family, 
                    mode, normalize){
  set.seed(seed)
  n_train = sample_size
  ...
  if (family == "binomial"){
    ...
    pr = 1/(1+exp(-lp))        
    Y = rbinom(n, 1, pr); table(Y)
    id = seq(1:n)
    #cases = id[Y == 1]
    cases = sample(id[Y == 1], length(id[Y == 1])); length(cases)
    #controls = sample(id[Y == 0], length(id[Y == 0]))
    controls = sample(id[Y == 0], length(id[Y == 0])); length(controls)
  ...
  dataset = list(G_train=G_train, G_valid=G_valid, G_test=G_test, 
                 ...
                 p=p, sample_size=sample_size, table_Y=table(Y), table_GxE=table(GxE), 
